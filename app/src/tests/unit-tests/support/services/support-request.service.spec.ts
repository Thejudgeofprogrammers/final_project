...
describe('SupportRequestService', () => {
    ...
    describe('findSupportRequests', () => {
        it('should return an array of support requests', async () => {
            ...
            const result = await service.findSupportRequests({ user: 'user1', isActive: true });
        ...
        it('should throw an error if findSupportRequests fails', async () => {
            ...
            await expect(service.findSupportRequests({ user: '123', isActive: true })).rejects.toThrow('Database error');
        });        
    });
    ...
    describe('sendMessage', () => {
        it('should send a message and emit an event', async () => {
            ...
            const sendMessageDto = { author: 'author-id', supportRequest: 'support-request-id', text: 'Hello' };
            ...
            expect(mockRequest.messages).toContainEqual(expect.objectContaining({ text: 'Hello' }));
            expect(mockRequest.save).toHaveBeenCalled();
            expect(eventEmitter.emit).toHaveBeenCalledWith('message.sent', mockRequest, expect.any(Message));
        });

        it('should throw an error if sendMessage fails', async () => {
            supportRequestModel.findById.mockRejectedValue(new Error('Database error'));
        
            await expect(
              service.sendMessage({ supportRequest: '123', author: 'user123', text: 'Hello' })
    ...
    describe('getMessages', () => {
        it('should return messages of a support request', async () => {
            ...
            expect(mockSupportRequestModel.findById).toHaveBeenCalledWith('support-request-id');
        });
        ...
        it('should throw an error if getMessages fails', async () => {
            ...
            await expect(service.getMessages('supportRequest123')).rejects.toThrow('Database error');
        });
    });
    ...
    describe('findAllByUser', () => {
        it('should return support requests filtered by userId', async () => {
            const mockRequests = [
                { _id: '1', userId: 'user1', createAt: new Date(), isActive: true, hasNewMessages: false },
            ];
            ...
            expect(result).toEqual(mockRequests.map(req => ({
                id: req._id,
                createAt: req.createAt,
                isActive: req.isActive,
                hasNewMessages: req.hasNewMessages,
            })));
            expect(mockSupportRequestModel.find).toHaveBeenCalledWith({ userId: 'user1', isActive: true });
        });

        it('should use default values for limit and offset', async () => {
            const mockRequests: any = [{ _id: '1', userId: 'user1', createAt: new Date(), isActive: true }];
            ...
            expect(mockSupportRequestModel.find).toHaveBeenCalledWith({ userId: 'user1', isActive: undefined });
        });
    ...
    describe('findAllForManager', () => {
        it('should return support requests for managers', async () => {
            const mockRequests = [
                {
                    ...
                    user: { _id: 'user1', name: 'User One', email: 'user1@example.com', contactPhone: '1234567890' },
                },
                {
                    _id: '2',
                    createAt: new Date(),
                    isActive: true,
                    hasNewMessages: true,
                    user: { _id: 'user2', name: 'User Two', email: 'user2@example.com', contactPhone: '0987654321' },
                },
            ];
            const query = { limit: 10, offset: 0, isActive: 'true' };
            ...
            expect(mockSupportRequestModel.find).toHaveBeenCalledWith({ isActive: true });
        });
