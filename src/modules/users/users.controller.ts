...
export class UsersController {
    ...
    async createUserAdmin(
        ...
        if (!email || !passwordHash || !name || !contactPhone || !role) throw new Error('Missing required fields');
        ...
        try {
            ...
            if (err instanceof ConflictException) throw new ConflictException('Email is already taken!');
            throw err;  
        };
    };
    ...
    async createUserManager(
        ...
        if (!email || !passwordHash || !name || !contactPhone) throw new Error('Missing required fields');
