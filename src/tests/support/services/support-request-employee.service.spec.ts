...
describe("SupportRequestEmployeeService", () => {
  ...
  describe("markMessagesAsRead", () => {
    ...
    it("should handle errors gracefully", async () => {
      const params = {
        supportRequest: "invalidId",
        user: "user123",
        createdBefore: new Date(),
      };
      supportRequestModelMock.findById.mockRejectedValue(
  ...
  describe("getUnreadCount", () => {
    ...
    it("should handle errors gracefully", async () => {
      ...
      await expect(service.getUnreadCount("invalidId")).rejects.toThrow(
        "Database error",
      );
    });
  });
  ...
  describe("closeRequest", () => {
    it("should close the support request", async () => {
      ...
      jest
        .spyOn(supportRequestModelMock, "findByIdAndUpdate")
        .mockResolvedValue(true);

      await service.closeRequest(supportRequestId);

      expect(supportRequestModelMock.findByIdAndUpdate).toHaveBeenCalledWith(
        supportRequestId,
        { isActive: false },
      );
    });
    ...
    it("should handle errors gracefully", async () => {
      supportRequestModelMock.findByIdAndUpdate.mockRejectedValue(
        new Error("Database error"),
      );

      await expect(service.closeRequest("invalidId")).rejects.toThrow(
        "Database error",
      );
    });
  });
