...
describe('CommonController', () => {
    ...
    beforeEach(async () => {
        ...
        supportRequestEmployeeService = module.get<SupportRequestEmployeeService>(SupportRequestEmployeeService);
    });
    ...
    describe('getSupportRequestMessages', () => {
        it('should return messages for a support request', async () => {
            ...
            const messages = await controller.getSupportRequestMessages(supportRequestId);

            expect(messages).toEqual(result);
            expect(mockSupportRequestService.getMessages).toHaveBeenCalledWith(supportRequestId);
        });
    });
    ...
    describe('sendSupportRequestMessage', () => {
        it('should send a message for a support request', async () => {
            ...
            const response = await controller.sendSupportRequestMessage(supportRequestId, createMessageDto, { user: mockUser });
    ...
    describe('markMessagesAsRead', () => {
        it('should mark messages as read', async () => {
            ...
            await controller.markMessagesAsRead(supportRequestId, { createdBefore }, { user: mockUser });

            expect(mockSupportRequestEmployeeService.markMessagesAsRead).toHaveBeenCalledWith({
                supportRequest: supportRequestId,
                user: userId,
