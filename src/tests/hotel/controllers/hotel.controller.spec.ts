...
describe('HotelController', () => {
    ...
    describe('searchHotelRooms', () => {
        it('should correctly map hotel ID when hotel is an ObjectId', async () => {
            ...
            const result = await hotelController.searchHotelRooms(10, 0, hotelId.toString());
        ...
        it('should handle invalid limit and offset values', async () => {
            ...
            const result = await hotelController.searchHotelRooms(-5, -1, mockRooms[0].hotel._id.toString());
    ...
    describe('searchHotelRoomById', () => {
        it('should return a hotel room with hotel as ObjectId', async () => {
            ...
            const result = await hotelController.searchHotelRoomById(mockRoom._id.toString());
            ...
            expect(hotelRoomService.findById).toHaveBeenCalledWith(mockRoom._id.toString());
        });
        ...
        it('should throw NotFoundException if hotel room not found', async () => {
            ...
            await expect(hotelController.searchHotelRoomById(new Types.ObjectId().toString())).rejects.toThrow(NotFoundException);

            expect(hotelRoomService.findById).toHaveBeenCalled();
        });

        it('should throw NotFoundException if ID is invalid', async () => {
            await expect(hotelController.searchHotelRoomById('invalid-id')).rejects.toThrow(NotFoundException);
        });
    });
});
