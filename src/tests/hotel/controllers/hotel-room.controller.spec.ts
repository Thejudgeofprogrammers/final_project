...
describe("HotelRoomController", () => {
  ...
  const mockHotelRoomService = {
    findByIdWithHotel: jest.fn().mockResolvedValue(mockHotelRoom),
    create: jest.fn().mockResolvedValue(mockHotelRoom),
  };
  beforeEach(async () => {
  ...
  describe("getHotels", () => {
    ...
    it("should return a list of hotels", async () => {
      ...
      const response = await controller.getHotels({
        limit: "10",
        offset: "0",
        title: "",
      } as any);
    ...
    it("should return a filtered list of hotels based on title", async () => {
      ...
      expect(result).toEqual([
        {
          id: mockHotels[0]._id.toString(),
          title: mockHotels[0].title,
          description: mockHotels[0].description,
        },
      ]);
      expect(hotelService.search).toHaveBeenCalledWith({
        limit: 10,
  ...
  describe("updateHotelDescription", () => {
    it("should update a hotel description", async () => {
      ...
      const result = await controller.updateHotelDescription(
        mockHotel._id.toString(),
        { title: mockHotel.title, description: "Updated description" },
      );
      expect(hotelService.update).toHaveBeenCalledWith(
        mockHotel._id.toString(),
        { title: mockHotel.title, description: "Updated description" },
      );
      expect(result).toEqual({
    ...
    it("should throw NotFoundException if hotel not found", async () => {
      ...
      await expect(
        controller.updateHotelDescription(mockHotel._id.toString(), {
          title: mockHotel.title,
          description: "Updated description",
        }),
      ).rejects.toThrow(NotFoundException);
    ...
    it("should throw NotFoundException if hotel ID is invalid", async () => {
      ...
      await expect(
        controller.updateHotelDescription(invalidId, updateHotelDTO),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe("addHotelRoom", () => {
    it("should create a hotel room with images", async () => {
      hotelService.findById = jest.fn().mockResolvedValue(mockHotel);
      mockHotelRoomService.create.mockResolvedValue(mockHotelRoom);
      mockHotelRoomService.findByIdWithHotel.mockResolvedValue(mockHotelRoom);
      const mockFiles = {
        images: [
          {
            fieldname: "images",
      ...
      const result = await controller.addHotelRoom(
        {
          hotelId: mockHotel._id.toString(),
          description: "Room description",
          isEnable: true,
        } as any,
        mockFiles,
