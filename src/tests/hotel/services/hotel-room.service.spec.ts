...
describe("HotelRoomService", () => {
  ...
  describe("create", () => {
    it("should create a hotel room", async () => {
      ...
      const result = await service.create(mockHotelRoom as any);
    ...
    it("should throw an error if create fails", async () => {
      ...
      await expect(
        service.create({ title: "New Room" } as Partial<HotelRoom>),
  ...
  describe("findById", () => {
    ...
    it("should throw an error if findById fails", async () => {
      ...
      await expect(service.findById(mockHotelRoom._id)).rejects.toThrow(
  ...
  describe("search", () => {
    ...
    it("should throw an error if search fails", async () => {
      ...
      await expect(
        service.search({ hotel: new Types.ObjectId(), offset: 0, limit: 10 }),
      ).rejects.toThrow("Database error");
    });
  });
  ...
  describe("update", () => {
    ...
    it("should throw an error if update fails", async () => {
      ...
      await expect(service.update(mockHotelRoom._id, {})).rejects.toThrow(
        "Update failed",
      );
    });
  });
