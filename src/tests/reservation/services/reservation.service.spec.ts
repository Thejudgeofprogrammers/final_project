...
import {
  Reservation,
  ReservationDocument,
} from "../../../modules/reservation/models/reservation.model";
...
describe("ReservationService", () => {
  ...
  beforeEach(async () => {
    ...
    model = module.get<Model<ReservationDocument>>(
  ...
  describe("addReservation", () => {
    ...
    it("should throw BadRequestException if room is already booked", async () => {
      ...
      await expect(service.addReservation(dto)).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("removeReservation", () => {
    it("should remove a reservation if it exists and user is authorized", async () => {
      ...
      jest.spyOn(model, "findById").mockResolvedValue(reservation);
      jest.spyOn(model, "findByIdAndDelete").mockResolvedValue(reservation);
    ...
    it("should throw BadRequestException if reservation does not exist", async () => {
      ...
      await expect(service.removeReservation(id, userId)).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("getReservations", () => {
    ...
    it("should throw an error if the database query fails", async () => {
      ...
      await expect(service.getReservations(filter)).rejects.toThrow(
        "Database error",
      );
    });
  });
