...
describe('ReservationManagerService', () => {
    ...
    describe('removeReservationByManager', () => {
        it('should remove a reservation by ID', async () => {
            ...
            mockReservationModel.findByIdAndDelete.mockResolvedValue(mockReservations[0]);
            ...
            expect(mockReservationModel.findByIdAndDelete).toHaveBeenCalledWith(reservationId);
        });
        ...
        it('should throw BadRequestException if reservation does not exist', async () => {
            ...
            await expect(service.removeReservationByManager(reservationId)).rejects.toThrow(BadRequestException);
            await expect(service.removeReservationByManager(reservationId)).rejects.toThrow('Reservation with the specified ID does not exist!');
        });
    });
});
