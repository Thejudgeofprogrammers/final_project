...
describe('ReservationManagerController', () => {
    ...
    beforeEach(async () => {
            ...
            providers: [
                {
                    ...
                    useValue: {
                        getReservationsByUser: jest.fn().mockResolvedValue(mockReservations),
        ...
        controller = module.get<ReservationManagerController>(ReservationManagerController);
    ...
    describe('removeReservationByManager', () => {
        it('should call removeReservationByManager with the correct ID', async () => {
            ...
            expect(service.removeReservationByManager).toHaveBeenCalledWith(reservationId);
        });

        it('should throw BadRequestException if the reservation does not exist', async () => {
            const reservationId: any = new Types.ObjectId();
            jest.spyOn(service, 'removeReservationByManager').mockRejectedValue(new BadRequestException('Reservation with the specified ID does not exist!'));

            await expect(controller.removeReservationByManager(reservationId)).rejects.toThrow(BadRequestException);
        });
    });
});
