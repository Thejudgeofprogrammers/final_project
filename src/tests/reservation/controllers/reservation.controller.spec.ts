...
describe("ReservationController", () => {
  ...
  describe("addReservation", () => {
    it("should add a reservation and return it", async () => {
      ...
      const result = await controller.addReservation(reservationDTO, {
        user: { _id: reservationDTO.userId },
      });
  ...
  describe("removeReservation", () => {
    it("should remove a reservation", async () => {
      ...
      await expect(
        controller.removeReservation(reservationId, { user: { _id: userId } }),
      ).resolves.not.toThrow();
      expect(service.removeReservation).toHaveBeenCalledWith(
        reservationId,
        userId,
      );
    });
  ...
  describe("getReservations", () => {
    it("should return an array of reservations", async () => {
      ...
      const result = await controller.getReservations(
        {
          dateStart: mockReservations[0].dateStart,
          dateEnd: mockReservations[0].dateEnd,
          userId: userId,
          hotelId: mockReservations[0].hotelId._id,
          roomId: mockReservations[0].roomId._id,
        },
        { user: { _id: userId } },
      );
      expect(result).toEqual(
